...
% figure(5)
% boxplot(HR_diff)
% ylabel('Heart Rate Difference (bpm)','FontSize',14)
% hold on
...
save('CoV','CoV')
save('PT','PT')
save('p_12_20','p_12_20')
% save('HR','HR')
% save('HR_diff','HR_diff')
% save('GSR','GSR')
% 
%%
subjectID = [1:subjectN 1:subjectN 1:subjectN 1:subjectN];
feedback = [ones(subjectN,1);2*ones(subjectN,1);ones(subjectN,1);2*ones(subjectN,1)];
muscle = [ones(subjectN*2,1);2*ones(subjectN*2,1)];
Data_CoV = [subjectID' feedback muscle CoV(:)];
save('Data_CoV','Data_CoV')
