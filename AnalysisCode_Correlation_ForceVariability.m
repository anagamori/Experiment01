...
for i = 1:subjectN 
    ...
    for j = 1:2
        ...
        PT_temp = [PT_All(:,1);PT_All(:,2)];
        PT_temp = zscore(PT_temp);
        CoV_stack = [CoV_stack;CoV_All(:,1)]; %;CoV_All(:,2)];
        %PT_stack = [PT_stack;PT_temp]; %PT_All(:,1);PT_All(:,2)];
        PT_stack = [PT_stack;PT_All(:,1)]; %;zscore(PT_All(:,2))];
        p_12_20_stack = [p_12_20_stack;p_12_20_All(:,1)];
        CoV_2_stack = [CoV_2_stack;CoV_All(:,2)]; %;CoV_All(:,2)];
        %PT_stack = [PT_stack;PT_temp]; %PT_All(:,1);PT_All(:,2)];
        PT_2_stack = [PT_2_stack;PT_All(:,2)]; %;zscore(PT_All(:,2))];
...
PT_stack_log = log([PT_stack;PT_2_stack]);
...
%%
p_12_20_stack_log = log([p_12_20_stack;p_12_20_2_stack]);
