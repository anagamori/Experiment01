...
%--------------------------------------------------------------------------
subjectFolder = '/Users/akiranagamori/Documents/GitHub/Experiment01/Record ID 01 20_MVC/';
dataFolder = '/Users/akiranagamori/Documents/GitHub/Experiment01/Record ID 01 20_MVC/Wrist flexion';
...
for i = 1:10
    ...
    if length(data.values_GSR(:,2)) < 23*Fs && length(data.values_GSR(:,2)) > 19*Fs
        ...
        successTrial_1 = [successTrial_1 i];
    ...
    if length(data.values_GSR(:,2)) < 23*Fs && length(data.values_GSR(:,2)) > 19*Fs
        ...
        successTrial_2 = [successTrial_2 i];
    elseif length(data.values_GSR(:,2)) < 19*Fs
 ...
 successTrial_2
%%
HR_max = max([max(max(HR_1_all(successTrial_1,:))),max(max(HR_2_all(successTrial_2,:)))]);
if HR_max > 150
    HR_max = 150;
end
HR_min = min([min(min(HR_1_all(successTrial_1,:))),min(min(HR_2_all(successTrial_2,:)))]);
...
hold on
ax_1_2 = plot(time,mean(HR_1_all(successTrial_1,:)),'color',[1 0 0],'LineWidth',2);
ax_2_2 = plot(time,mean(HR_2_all(successTrial_2,:)),'color',[0 25 255]/255,'LineWidth',2);
...
GSR_max = max([max(max(GSR_1_all(successTrial_1,:))),max(max(GSR_2_all(successTrial_2,:)))]);
GSR_min = min([min(min(GSR_1_all(successTrial_1,:))),min(min(GSR_2_all(successTrial_2,:)))]);
figure(2)
xlabel('Time (sec)','FontSize',14)
ylabel('GSR','FontSize',14)
ylim([GSR_min-5 GSR_max+5])
hold on
ax_1_2 = plot(time,mean(GSR_1_all(successTrial_1,:)),'color',[1 0 0],'LineWidth',2);
ax_2_2 = plot(time,mean(GSR_2_all(successTrial_2,:)),'color',[0 25 255]/255,'LineWidth',2);
